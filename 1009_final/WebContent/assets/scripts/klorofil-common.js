$(document).ready(function() {
	...
	var affectedElement = $('.panel-action-body');
	...
	$('.panel .btn-toggle-collapse').clickToggle(
			...
			function(e) {
				...
				// if has scroll
				if( $(this).parents('.panel-action').find('.slimScrollDiv').length > 0 ) {
					affectedElement = $('.slimScrollDiv');
				}

				$(this).parents('.panel-action').find(affectedElement).slideDown(300);
		...
		function(e) {
			...
			$(this).parents('.panel-action').find(affectedElement).slideUp(300);
			$(this).find('i.lnr-chevron-up').toggleClass('lnr-chevron-down');
		}
	);
...
//panel update
$('.panel .btn-update').click(function(e){

	e.preventDefault();
	$(this).parents('.panel-heading').find('.form-reply').attr("readonly",false).focus();
